...
export class FlashcardsCardsComponent implements OnInit {
  ...
  editFlashcard(intent: 'edit' | 'save' | 'cancel'): void {
    let buttons = {
      edit: document.querySelectorAll('.flashcard-card > .action > .edit')[
        this.id
      ],
      save: document.querySelectorAll('.flashcard-card > .action > .save')[
        this.id
      ],
      // cancel: document.querySelectorAll('.flashcard-card > .action > .cancel')[this.id],
    }
    let flashcard = document.querySelectorAll('mat-card-content > .content')[
      this.id
    ]
    let editForm = document.querySelectorAll('mat-card-content > .editForm')[
      this.id
    ]
