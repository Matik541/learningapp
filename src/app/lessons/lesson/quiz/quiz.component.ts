...
export class QuizComponent implements OnInit {
	...
	ngOnInit(): void {
		this.lec = this.lessonsService.getLesson(this.id).subscribe((data) => {
			if (data != null) {
				this.lesson = data;
				this.quizQuestions.write = this.setWrite(Math.min(5, this.lesson.flashcards.length));
				this.quizQuestions.match = this.setMatch(Math.min(5, this.lesson.flashcards.length));
				this.quizQuestions.choose = this.setChoose(Math.min(5, this.lesson.flashcards.length));
			}
		});
	}
	ngOnDestroy() {
	...
	submit(): void {
		...
		this.quizQuestions.write.forEach((question, index) => {
			let flashcard = this.lesson.flashcards.find((card) => card.question === question);
			if (flashcard) {
				// if (flashcard.answer === this.answers[0][index]) correct++;
				// else if (this.answers[0][index] != undefined && this.answers[0][index] != '') incorrect++;
				allAnswers[0].push({
					question,
		...
		this.quizQuestions.match.forEach((question, index) => {
			let flashcard = this.lesson.flashcards.find((card) => card.question === question.question);
			if (flashcard) {
				allAnswers[1].push({
		...
		this.quizQuestions.choose.forEach((question, index) => {
			...
			if (flashcard) {
				...
				if (
					(this.answers[2][index] == 'true' && flashcard.answer === question.answer) ||
					(this.answers[2][index] == 'false' && flashcard.answer !== question.answer)
				)
					correctAnswer = true;
